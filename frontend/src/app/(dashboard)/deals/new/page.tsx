...
export default function NewDealPage() {
  ...
  const [formData, setFormData] = useState({
    dealName: '', 
    items: [{ itemName: '', quantity: 1, costPricePerItem: 0 }],
    flightCost: 0, luggageCost: 0, baggageAllowanceCost: 0, transportCost: 0, packageCost: 0,
  ...
  const totalStockCost = formData.items.reduce((sum, item) => sum + ((Number(item.quantity) || 0) * (Number(item.costPricePerItem) || 0)), 0);
